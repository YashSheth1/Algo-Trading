...
def market(MACD_data_objects,order_type,size,price,ratio):
    ...
    for current_item in MACD_data_objects:
        ...
        #wait till all values are available
        if current_item.sp!=0 and current_item.esp!=0:
            ...
            if datetime.strptime(candle_time_end,"%H:%M:%S")<datetime.strptime('18:00:00',"%H:%M:%S") and datetime.strptime('07:00:00',"%H:%M:%S")<=datetime.strptime(candle_time_start,"%H:%M:%S")<=datetime.strptime('19:00:00',"%H:%M:%S"):
                ...
                if current_item.emacd1>=current_item.esp and previous_macd<previous_signal :
                    
                    if position==0 and volume_ema_fast[iterator_for_ema]>volume_ema_slow[iterator_for_ema]:
                        print "BUY 0 to 1"
                        print volume_ema_fast[iterator_for_ema]
                        print volume_ema_slow[iterator_for_ema]
                        ...
                        #only if fast EMA > slow EMA then 1 long
                        
                        if volume_ema_fast[iterator_for_ema]>volume_ema_slow[iterator_for_ema]:
                            print "BUY -1 to 1"
                            print volume_ema_fast[iterator_for_ema]
                            print volume_ema_slow[iterator_for_ema]
                            ...
                            #only if fast EMA > slow EMA then position 0
                            print "BUY -1 to 0"
                            print volume_ema_fast[iterator_for_ema]
                            print volume_ema_slow[iterator_for_ema]
                ...
                elif previous_macd>previous_signal and current_item.emacd1<=current_item.esp :                    
                    if position==1:
                        #when pos=1 and fastEMA<slowEMA hence sell 1 extra lot
                        print "SEll 1 to -1"
                        print volume_ema_fast[iterator_for_ema]
                        print volume_ema_slow[iterator_for_ema]
                        if volume_ema_fast[iterator_for_ema]<volume_ema_slow[iterator_for_ema]:
                            ...
                            #when pos=1 and fastEMA<slowEMA hence sell lots and pos=0
                            print "SELL 1 to 0"
                            print volume_ema_fast[iterator_for_ema]
                            print volume_ema_slow[iterator_for_ema]
                            new_day_indicator+=1
                            ...
                            if indicator==9:
                                ...
                                position=0
                    #when pos=0 and fastEMA<slowEMA hence sell 1 extra pos=-1        
                    elif position==0 and volume_ema_fast[iterator_for_ema]<volume_ema_slow[iterator_for_ema]:
                        print "SELL 0 to -1"
                        print volume_ema_fast[iterator_for_ema]
                        print volume_ema_slow[iterator_for_ema]
            ...
            previous_macd=current_item.emacd1
            previous_signal=current_item.esp
            #Uncomment For Graph generation !!
            """
            time_reference.append(str(current_item.period.time_end))
            macd_values_for_graph.append(current_item.emacd1)
            signal_values_for_graph.append(current_item.esp)"""
    	iterator_for_ema+=1    
...
volume_ema_fast_iterator=obj.calculation(3,config_data_main['volume_fast_ema'],session)
volume_ema_slow_iterator=obj.calculation(3,config_data_main['volume_slow_ema'],session)

for item in volume_ema_slow_iterator:
    volume_ema_slow.append(item.ema)
for item in volume_ema_fast_iterator:
    volume_ema_fast.append(item.ema)
...
#market(MACD_Values,'Market',1,0.0,0.0)
print trades
